...
<body>
    ...
    <SCRIPT LANGUAGE="JavaScript">
        function linkClicked(s1, s2)
        {
            // refreshes the page with a link selected
            var form = "form"+s1+"x"+s2;
            var circle = "circle"+s1+"x"+s2;
    ...
    <div id="page-wrapper">
            ...
            <div class="container-fluid">
                ...
                <div class="row">
                    <div class="col-lg-6">
                        <h2>Your Circle Requests</h2>
                        <div class="table-responsive">
                            <table class="table table-bordered table-hover">
                                <thead>
                                    <tr>
                                        <%
                                        // display the circle requests for circles that this user owns
                                        String customerID = session.getAttribute("login").toString();
                                        String Query = 
                                            "SELECT	CircleID, CustomerID, FirstName, LastName, CircleName" +
                                            " FROM      CircleRequest NATURAL JOIN Circle NATURAL JOIN Customer" +
                                        ...
                                        {
                                        %>
                                        <th> Circle Name </th><th> User </th>
                                        <%
                                        } else
                                        {
                                        %>
                                        <th> You have no pending requests </th>
                                        <%
                                        }
                                        %>
                                    </tr>
                                ...
                                %>
                                    ...
                                    <form id="<% out.print("form" + rs.getString(1)+"x"+rs.getString(2));%>" method="POST">
                                    <input  type="HIDDEN"
                                        name="<% out.print("circle" + rs.getString(1)+"x"+rs.getString(2));%>"
                                        id="<% out.print("circle" + rs.getString(1)+"x"+rs.getString(2));%>">
                                    <tr ONCLICK="linkClicked(<% out.print(rs.getString(1) + "," + rs.getString(2));%>) ">
                                            <td > <% out.print(rs.getString(5)); %> </td>
                                            <td > <% out.print(rs.getString(3) + " " + rs.getString(4)); %> </td>
                                        </form>
                                    </tr>
                                <%
                                    ...
                                    if (request.getParameter("circle" + rs.getString(1)+"x"+rs.getString(2))!=null)
                                    {
                                        // process the circle request
                                        Query = "INSERT CircleMembership(CustomerID, CircleID)"
                                                + " Values('"+rs.getString(2)+"','"+rs.getString(1)+"');"
                                                ;
                                        int success = DBConnection.ExecUpdateQuery(Query);

                                        Query = "DELETE FROM CircleRequest"
                                                + " where CustomerID='"+rs.getString(2)+"'"
                                                + " AND CircleID='"+rs.getString(1)+"';"
                                                ;
                                        
                                        success = DBConnection.ExecUpdateQuery(Query);
                                        
                                        // refresh this page
                                        response.sendRedirect("AcceptCircleRequests.jsp");
                                        return;
                                    }
                                }
                                %>
